...
def correct_s2_tile_prediction(h5_fp: str, mgrs_tiles: str, s2_fp: str, s2_tiles: str, rh_idx: int=98, q_idx: int=1):
    '''
    Correct S2 tile prediction
    '''
    df = get_gedi_from_h5(h5_fp, mgrs_tiles, s2_fp, s2_tiles)
    for tile_id in df['Name'].unique():
        df_tile = df[df['Name'] == tile_id]
        ...
        with rasterio.open(correct_fp, 'w', **profile) as dst:
            ...
            correct_pred = a * raw_pred + b
            correct_pred = correct_pred.round()
...
def main(cfg):
    ...
    correct_s2_tile_prediction(cfg.h5_fp, cfg.mgrs_tiles, cfg.s2_fp, cfg.s2_tiles)
