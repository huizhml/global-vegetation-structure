...
class VSMCorrection:
    def __init__(self, year: int, root_output_dir: str, **kwargs):
        self.year = year
        self.root_output_dir = Path(root_output_dir).expanduser()

    def run_correction_and_blending(self):
        '''
        Run correction and blending for all tiles within the GEDI coverage.
        Pre-requisite:
        - Generate distance maps for all tiles
        - Calculate residuals for all tiles, and save the residuals for each RH for later correction step.


        '''
        pass

    def generate_distance_maps(self, distance_map_dir_name: str = 'distance_maps', stac_collection_dir: str = None):
        '''
        Generate distance maps for all tiles
        Args:
            distance_map_dir_name: str, the name of the directory to save the distance maps
        '''
        distance_map_dir = self.root_output_dir / distance_map_dir_name
        distance_map_dir.mkdir(parents=True, exist_ok=True)
        stac_collection_dir = Path(stac_collection_dir).expanduser()

        def create_distance_map(tile_id: str):
            '''
            Create distance map (geotiff) for a tile. Needs shape, crs, and transform.
            shape for all Sentinel-2 tiles is (10980, 10980)
            '''
            item = pystac.Item.from_file(str(stac_collection_dir / tile_id / f'{tile_id}.json'))
            ...
            distance_arr = create_distance_arr(shape)
            distance_map_path = distance_map_dir / f'{tile_id}.tif'
            with rasterio.open(distance_map_path, 'w', **profile) as dst:
                dst.write(distance_arr)

        s2_tiles = glob.glob(str(stac_collection_dir / '*.json'))
        for s2_tile in s2_tiles:
            create_distance_map(s2_tile.stem)
