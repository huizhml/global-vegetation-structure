...
def extract_pred_points(
    ...
    ours_rh_cols = [f'RH{i}_Q{j}' for i in rh_idxs for j in range(1,2)]
    ...
    def exe_one_tile(tile_id: str):
        ...
        for rh_idx in rh_idxs:
            for q_idx in range(1,2):
                pred_fp = pred_dir / f'RH{rh_idx}_Q{q_idx}.tif'
...
def _extract_vsm_patch_from_tile(stac_collection_dir: str, tile_id: str, year: int, save_dir: str, loc_df: pd.DataFrame, rh_idxs: List[int] = np.arange(101), q_idxs: List[int] = [1], **kwargs):
    '''
    Extract the VS prediction patches from the tile
    Parameters:
    ...
    ours_rh_cols = [f'RH{i}_Q{j}_raw' for i in rh_idxs for j in q_idxs]
    preds = []
    for rh_idx in rh_idxs:
        for q_idx in q_idxs:
            pred_fp = pred_dir / f'RH{rh_idx}_Q{q_idx}.tif'
    ...
    preds = np.concatenate(preds, axis=1) # (n_points, len(ours_rh_cols))
    preds = preds.astype(np.float32)
    preds = np.where(preds == nodata, np.nan, preds) # mask nodata (non-vegetation) for pred and ref
    
    _df = pd.DataFrame(preds/10, index=loc_df.index, columns=ours_rh_cols)
    df = loc_df.join(_df)
    df = gpd.GeoDataFrame(df, geometry='geometry', crs="EPSG:4326")
    df.to_parquet(save_dir / f'{tile_id}.parquet')
    return df

def extract_pred_patches(loc_path: str,  save_dir: str, lat_col: str = 'lat', lon_col: str = 'lon', s2_grid_file: str = None, **kwargs):
    '''
    Parameters:
        loc_path: path to the location file, can be a csv/parquet file or directory containing csv/parquet files
        save_dir: path to save the extracted patches
        lat_col: name of the latitude column
        lon_col: name of the longitude column
        s2_grid_file: path to the S2 grid file
    ...
    '''
    stac_collection_dir = Path(f'~/data/gvs/products/gvsm_stac_catalog/vsm_local').expanduser()
    loc_path = Path(loc_path).expanduser()
    save_dir = Path(save_dir).expanduser()
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    if loc_path.is_dir():
        loc_files = list(loc_path.glob('*.csv')) + list(loc_path.glob('*.parquet'))
        df = dd.read_csv(loc_files) # TODO:
    else:
        df = pd.read_csv(loc_path) # TODO: add support for parquet files
    ...
    df = df[df['covered_by_gedi'] == True]
    
    df.groupby('Name').apply(lambda x: x.to_parquet(save_dir / f'{x.Name.iloc[0]}.parquet'))
