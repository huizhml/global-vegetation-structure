...
class RobustLoss(nn.Module):
    ...
    """
    def __init__(self, approximate=False, epsilon=1e-6):
    ...
    def forward(self, x, scale, alpha):
        ...
        if self.approximate:
            ...
            assert self.epsilon > np.finfo(np.float32).eps
            # Compute an approximate form of the loss which is faster, but innacurate
            # when x and alpha are near zero.
            b = torch.abs(alpha - 2) + self.epsilon
            d = torch.where(alpha >= 0, alpha + self.epsilon, alpha - self.epsilon)
