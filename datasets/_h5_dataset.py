...
def calculate_mean_std(h5_file: str, mean_std_fp: str):
    import xarray as xr
    h5_file = Path(h5_file).expanduser()
    mean_std_fp = Path(mean_std_fp).expanduser()
    ds = xr.open_dataset(h5_file, engine='h5netcdf')
    rhs_mean = ds.rhs_median.mean(dim=['loc', 'y', 'x'])
    rhs_std = ds.rhs_median.std(dim=['loc', 'y', 'x'])
    s2_mean = ds.s2.mean(dim=['loc', 'y', 'x'])
    s2_std = ds.s2.std(dim=['loc', 'y', 'x'])
    np.savez(mean_std_fp, mean=rhs_mean, std=rhs_std, mean_s2=s2_mean, std_s2=s2_std)

def get_reference_data_with_images(h5_file: str, naturalness_fp: str):
    h5_file = Path(h5_file).expanduser()
    ds = xr.open_dataset(h5_file, engine='h5netcdf')
    rowid_list = ds.rowid.values
    naturalness_fp = Path(naturalness_fp).expanduser()
    target_df = pd.read_csv(naturalness_fp, index_col='rowid')
    target_df_with_images = target_df.loc[rowid_list]
