...
class VisRHS:
    ...
    def make_gif(self):
        ...
        for tile_info in cfg['tiles']: # tile-specific parameters
            ...
            if n_pngs >= len(rh_idxs): 
                continue
            window = rio.windows.Window(col_off=tile_info['col_off'], row_off=tile_info['row_off'], width=cfg['width'], height=cfg['height'])
    ...
    def vis_rgb(self):
        ...
        for time in cfg['time_idxs']:
            ...
            rgb = ds.s2.sel(band=['B04', 'B03', 'B02']).isel(time=time, x=slice(cfg['col_off'], cfg['col_off']+cfg['width']), y=slice(cfg['row_off'], cfg['row_off']+cfg['height']))
    ...
    def vis_rhs(self):
        ...
        for rh_idx in cfg['rh_idxs']:
            window = rio.windows.Window(col_off=cfg['col_off'], row_off=cfg['row_off'], width=cfg['width'], height=cfg['height'])
            file_path = self.prediction_dir / f"{cfg['tile_id']}_cog/RH{rh_idx}_Q1.cog.tif"
    ...
    def vis_rhs_3d(self, tile, rh_idx, window, gif_png_dir, z_exaggeration=2.0, camera=None, aspectratio=(1,1,0.25), cmin=None, cmax=None, orthographic=False, title=None):
