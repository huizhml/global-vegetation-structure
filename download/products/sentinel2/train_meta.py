...
class S2MetaGather(DaskDownloader):
    ...
    def process_zone(self):
        ...
        * zone (str or list): The MGRS zone(s) to filter S2 tiles for. GEDI partitions with S2 candidates added will be saved to self.save_dir / zone.
        ...
        logger.info(f'processing zone: {self.zone}')
        print(client)
        if isinstance(self.zone, str):
            ...
            self.temp_dir_zone = self.temp_dir / f'{self.year}/{self.zone}'
            s2_table_file = self.S2_meta_dir / f'{self.year}/{self.zone}.parquet'
        else:
            # Read GEDI partitions from multiple zones, add paths of partitions in divisions
            self.temp_dir_zone = self.temp_dir / f'{self.year}/small_zones'
            s2_table_file = self.S2_meta_dir / f'{self.year}/small_zones.parquet'
            files = []
            for z in self.zone:
        ...
        self.target_files = [self.save_dir / f'{self.year}/{self.zone}/{f.stem}.parquet'  for f in files]
        files = [str(p) for p in files]
        self.files = sorted(files, key=natural_sort_key)
        if not self.rewrite:
            for f in self.target_files:
                if f.exists():
                    self.files.remove(str(self.gedi_dir / f'{self.year}/{self.zone}/{f.stem}.parquet'))

        if self.files == []:
            logger.info(f'All partitions for zone {self.zone} have S2 candidates. Skipping...')
            return
